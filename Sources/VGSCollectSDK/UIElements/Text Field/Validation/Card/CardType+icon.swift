...
extension VGSPaymentCards.CardBrand {
    ...
    var brandIcon: UIImage? {
      return VGSPaymentCards.validCardBrands.first(where: { $0.brand == self })?.brandIcon ?? VGSPaymentCards.unknown.brandIcon
    }
  
    var cvcIcon: UIImage? {
      return VGSPaymentCards.validCardBrands.first(where: { $0.brand == self })?.cvcIcon ?? VGSPaymentCards.unknown.cvcIcon
    }
